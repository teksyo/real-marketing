  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      await logActivity(
        ...
        user.id,
        "LEAD_CREATED",
        `Lead created manually for ${address || zipCode}${
          phoneNumber ? ` with phone ${phoneNumber}` : ""
        }`,
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (phoneNumber && phoneNumber.trim()) {
        ...
        const existingContact = existingLead.contacts.find(
          (c) => c.phoneNumber === trimmedPhone
        );
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      // Log note activity
      await logActivity(leadId, user.id, "NOTE_ADDED", note, {
    ...
    try {
      ...
      await logActivity(
        ...
        `Follow-up scheduled for ${new Date(
          followUpDate
        ).toLocaleDateString()}${notes ? `: ${notes}` : ""}`,
        {
          followUpDate,
          notes,
        }
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const leads = await prisma.lead.findMany({
        where: {
          region: {
            contains: `, ${region}`, // This will match any region containing ", CA"
          },
          OR: [
