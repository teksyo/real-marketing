...
// ZIP codes by state abbreviation
const stateZipCodes = {
  'AL': '35201',
  'AK': '99501',
  'AZ': '85001',
...
const USAMap = ({ onRegionSelect }) => {
  ...
  // Layer style for state boundaries
  const stateLayer = {
    id: 'states-fill',
    type: 'fill',
    paint: {
  ...
  if (!MAPBOX_TOKEN) {
    return (
      <div className="w-full h-[600px] rounded-lg overflow-hidden flex items-center justify-center bg-gray-100">
        <p className="text-gray-500">Please add your Mapbox token to .env.local</p>
      </div>
    );
  }
